    ...
    public class Octree {
        ...
        private class OctNode {
            ...
            bool IsMonotone() {
                ...
                OctNodeData first = children[0].data;
                for (int b = 1; b < 8; b++) {
                    OctNodeData curr = children[b].data;
                    if (first.signedDist == 0 && curr.signedDist == 0) {
                        if (first.type > 0 != curr.type > 0) return false;
                    } else {
                        if (first.type != curr.type || first.signedDist != curr.signedDist) return false;
                    }
                }
